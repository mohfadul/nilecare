...
// JWT Payload interface (received from Auth Service)
 ...
 */
function validateAuthConfig(): { authServiceUrl: string; serviceApiKey: string; serviceName: string } {
  ...
  if (!authServiceUrl) {
    throw new Error(
      'CRITICAL: AUTH_SERVICE_URL environment variable is not set. ' +
  ...
  if (!serviceApiKey) {
    throw new Error(
      'CRITICAL: AUTH_SERVICE_API_KEY environment variable is not set. ' +
  ...
  try {
    ...
    try {
      const response = await axios.post(
        `${authServiceUrl}/api/v1/integration/validate-token`,
        { token },
        {
          headers: {
...
export function requireRole(roles: string | string[]) {
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!allowedRoles.includes(req.user.role)) {
      console.warn(
        `[Auth] Access denied: user=${req.user.userId}, role=${req.user.role}, required=${allowedRoles.join(', ')}`
      );
...
export function requirePermission(permission: string) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const response = await axios.post(
        `${authServiceUrl}/api/v1/integration/verify-permission`,
        {
      ...
      if (!allowed) {
        console.warn(
          `[Auth] Permission denied: service=${serviceName}, user=${req.user.userId}, ` +
      ...
      // Permission granted
      console.log(
        `[Auth] Permission granted: service=${serviceName}, user=${req.user.userId}, permission=${permission}`
      );
 ...
 */
export function getServiceAuthHeaders(userToken?: string): Record<string, string> {
