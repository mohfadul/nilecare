...
export interface MeasureDefinition {
  ...
  benchmark: number; // Target performance rate (0-1)
  reportingPeriod: 'monthly' | 'quarterly' | 'annual';
  sudanMoHRequired: boolean; // Required by Sudan Ministry of Health
}
...
export class QualityMeasureService {
  ...
  private readonly SUDAN_MOH_MEASURES: MeasureDefinition[] = [
    {
      measureId: 'SUDAN-001',
   ...
   */
  async calculateMeasurePerformance(
    measureId: string,
    facilityId: string,
    startDate: Date,
    endDate: Date
   ...
   */
  private async getMeasureDefinition(measureId: string): Promise<MeasureDefinition | null> {
   ...
   * Calculate denominator (eligible population)
   */
  private async calculateDenominator(
   ...
   * Calculate numerator (meeting criteria)
   */
  private async calculateNumerator(
    measure: MeasureDefinition,
    facilityId: string,
    startDate: Date,
    endDate: Date,
   ...
   * Calculate exclusions
   */
  private async calculateExclusions(
    measure: MeasureDefinition,
    facilityId: string,
    startDate: Date,
    endDate: Date,
  ...
  private getPerformanceStatus(
    ...
    benchmark: number,
    measureId: string
  ): 'exceeds' | 'meets' | 'below' | 'critical' {
    // For measures where lower is better (e.g., readmission rate)
    const lowerIsBetter = ['SUDAN-005', 'SUDAN-007'].includes(measureId);

    if (lowerIsBetter) {
      if (performanceRate <= benchmark * 0.8) return 'exceeds';
      if (performanceRate <= benchmark) return 'meets';
      if (performanceRate <= benchmark * 1.2) return 'below';
      return 'critical';
    } else {
      if (performanceRate >= benchmark * 1.05) return 'exceeds';
      if (performanceRate >= benchmark) return 'meets';
      if (performanceRate >= benchmark * 0.9) return 'below';
      return 'critical';
    }
  }
  ...
  private async calculateTrend(
    ...
    if (avgChange > 0.02) return 'improving';
    if (avgChange < -0.02) return 'declining';
    return 'stable';
  }
   ...
   */
  private async storeMeasureResult(result: MeasureResult): Promise<void> {
    const query = `
  ...
  async generateQualityDashboard(
    ...
    for (const measureDef of this.SUDAN_MOH_MEASURES) {
      try {
        const result = await this.calculateMeasurePerformance(
          measureDef.measureId,
          facilityId,
          startDate,
          endDate
        );
    ...
    const mohMeasures = measures.filter(m => 
      this.SUDAN_MOH_MEASURES.find(def => 
        def.measureId === m.measureId && def.sudanMoHRequired
      )
    );
    const sudanMoHCompliance = mohMeasures.every(m => 
  ...
  async generateSudanMoHReport(
    ...
    const mohMeasures = dashboard.measures.filter(m =>
      this.SUDAN_MOH_MEASURES.find(def =>
        def.measureId === m.measureId && def.sudanMoHRequired
      )
    );
   ...
   */
  private generateRecommendations(measures: MeasureResult[]): string[] {
      ...
      if (measure.status === 'below' || measure.status === 'critical') {
        ...
        if (measureDef) {
          recommendations.push(
            `Improve ${measureDef.measureName}: Current ${(measure.performanceRate * 100).toFixed(1)}%, Target ${(measureDef.benchmark * 100).toFixed(1)}%`
          );
        }
      }
    });
