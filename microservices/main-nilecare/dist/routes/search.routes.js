...
router.post('/patients', async (req, res) => {
    ...
    try {
        const { searchTerm, gender, minAge, maxAge, bloodType, city, state, hasAllergies, hasMedicalConditions, dateFrom, dateTo, page = 1, limit = 10, } = req.body;
...
router.post('/appointments', async (req, res) => {
    const connection = await getConnection();
    try {
        const { searchTerm, patientId, providerId, status, statuses, // Array of statuses
        dateFrom, dateTo, duration, appointmentType, page = 1, limit = 10, } = req.body;
        let query = `
      SELECT 
        a.*, 
...
router.post('/users', async (req, res) => {
    const connection = await getConnection();
    try {
        const { searchTerm, role, roles, // Array of roles
        status, specialty, department, page = 1, limit = 10, } = req.body;
