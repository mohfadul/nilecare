...
router.get('/summary', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Failed actions
    const [failedResult] = await connection.execute(
    ...
    const [entityTypes] = await connection.execute(
    ...
    res.json({
      success: true,
      data: {
        totalActions: (totalResult as any[])[0].total,
        failedActions: (failedResult as any[])[0].total,
        actionTypes,
        activeUsers,
        entityTypes,
      },
 ...
 */
router.get('/data-exports', async (req: Request, res: Response) => {
  const connection = await getConnection();
