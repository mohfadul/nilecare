# appointment-service Implementation Instructions

## File: microservices\appointment-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\appointment-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
appointment_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3016, () => {
      console.log('✅ appointment-service started on port 3016');
      console.log('✅ Ready: http://localhost:3016/health/ready');
      console.log('✅ Metrics: http://localhost:3016/metrics');
    });
  } catch (error) {
