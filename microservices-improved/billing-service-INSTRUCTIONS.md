...
## File: microservices\billing-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\billing-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
billing_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3015, () => {
      console.log('✅ billing-service started on port 3015');
      console.log('✅ Ready: http://localhost:3015/health/ready');
      console.log('✅ Metrics: http://localhost:3015/metrics');
    });
  } catch (error) {
