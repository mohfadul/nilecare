# facility-service Implementation Instructions

## File: microservices\facility-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\facility-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
facility_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3012, () => {
      console.log('✅ facility-service started on port 3012');
      console.log('✅ Ready: http://localhost:3012/health/ready');
      console.log('✅ Metrics: http://localhost:3012/metrics');
    });
  } catch (error) {
