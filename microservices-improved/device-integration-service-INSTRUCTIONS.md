# device-integration-service Implementation Instructions

## File: microservices\device-integration-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\device-integration-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
device_integration_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3011, () => {
      console.log('✅ device-integration-service started on port 3011');
      console.log('✅ Ready: http://localhost:3011/health/ready');
      console.log('✅ Metrics: http://localhost:3011/metrics');
    });
  } catch (error) {
