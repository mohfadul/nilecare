# inventory-service Implementation Instructions

## File: microservices\inventory-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\inventory-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
inventory_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3013, () => {
      console.log('✅ inventory-service started on port 3013');
      console.log('✅ Ready: http://localhost:3013/health/ready');
      console.log('✅ Metrics: http://localhost:3013/metrics');
    });
  } catch (error) {
