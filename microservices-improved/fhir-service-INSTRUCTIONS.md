# fhir-service Implementation Instructions

## File: microservices\fhir-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\fhir-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
fhir_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3009, () => {
      console.log('✅ fhir-service started on port 3009');
      console.log('✅ Ready: http://localhost:3009/health/ready');
      console.log('✅ Metrics: http://localhost:3009/metrics');
    });
  } catch (error) {
