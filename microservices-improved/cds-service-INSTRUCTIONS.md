# cds-service Implementation Instructions

## File: microservices\cds-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\cds-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
cds_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3008, () => {
      console.log('✅ cds-service started on port 3008');
      console.log('✅ Ready: http://localhost:3008/health/ready');
      console.log('✅ Metrics: http://localhost:3008/metrics');
    });
  } catch (error) {
