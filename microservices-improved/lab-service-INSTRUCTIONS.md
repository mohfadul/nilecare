# lab-service Implementation Instructions

## File: microservices\lab-service\src\index.ts

### Step 1: Backup created
✅ Backup at: microservices\lab-service\src\index.ts.backup
...
app.get('/metrics', (req, res) => {
  ...
  res.send(`
lab_service_uptime_seconds ${Math.floor((Date.now() - serviceStartTime) / 1000)}
...
(async () => {
  try {
    ...
    server.listen(3006, () => {
      console.log('✅ lab-service started on port 3006');
      console.log('✅ Ready: http://localhost:3006/health/ready');
      console.log('✅ Metrics: http://localhost:3006/metrics');
    });
  } catch (error) {
